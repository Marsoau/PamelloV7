...
public class OsuUserPlatform : IUserPlatform
{
    private readonly OsuClientService _client;
    
    public string Name => "osu";

    public OsuUserPlatform(IServiceProvider services) {
    ...
    public string ValueToKey(string value) {
        ...
        if (uri.Host != "osu.ppy.sh" || uri.Segments[1] != "users/") throw new PamelloException($"Cant find a valid osu link in value \"{value}\"");
