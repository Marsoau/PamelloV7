...
namespace PamelloV7.Module.Marsoau.Discord.Commands.Modals.Favorite;
    ...
    public async Task Submit(string userQuery, ESongOrPlaylist songOrPlaylist) {
        ...
        switch (songOrPlaylist) {
            case ESongOrPlaylist.Song: {
                var newSongs = _peql.Get<IPamelloSong>(newItemsQueries, User);
                user.ReplaceFavoriteSongs(newSongs);
            } break;
            case ESongOrPlaylist.Playlist: {
                var newPlaylists = _peql.Get<IPamelloPlaylist>(newItemsQueries, User);
