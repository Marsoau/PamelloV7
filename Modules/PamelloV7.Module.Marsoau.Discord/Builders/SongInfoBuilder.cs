...
{
    public async Task<ComponentBuilderV2> Component(IPamelloSong song) {
        ...
        foreach (var source in song.Sources) {
            var emote = await clients.GetEmote(source.PK.Platform);
            var line = emote is null
                ? $"{source.PK.Platform}: {DiscordString.Url(source.PK.Key, source.GetUrl())}"
        ...
        var componentBuilder = new ComponentBuilderV2()
            .WithContainer(containerBuilder = new ContainerBuilder()
                ...
                .WithActionRow(new ActionRowBuilder()
                    ...
                    .WithButton(new ButtonBuilder()
                        .WithCustomId($"pamello-command:SongInfoReset?song={song.Id}")
                        .WithLabel("Reset")
                        .WithStyle(ButtonStyle.Secondary)
                    )
                )
                ...
                .WithSection(new SectionBuilder()
                    .WithAccessory(new ButtonBuilder()
                        .WithCustomId($"song-source-select:{song.Id}")
                        .WithLabel("Select")
                        .WithStyle(ButtonStyle.Secondary)
                    )
                    .WithTextDisplay($"""
